...
c. 角色去重校验 → 对叙事摘要中提到的每个角色：
   ...
   3. 去重判断：
      ...
      - 确认为新角色 → 创建新角色文件
d. 写入场记 → \`.novel/scenes/sXXX.md\`（格式见下）
e. 更新角色 → 对每个有变化的角色，读取 + 编辑 .novel/characters/*.md
f. 更新 .novel/world.md → 从叙事中提取地点描述和世界规则，编辑 .novel/world.md
g. 更新 .novel/plot.md → 从叙事中提取剧情事件，编辑 .novel/plot.md
h. 更新 .novel/timeline → 编辑 .novel/timeline.md
i. 处理传播债务 → 每条新事实为每个受影响文件创建债务，编辑 .novel/debts.md
j. 读取目录，验证文件是否更新正确，如果没有则重新生成
